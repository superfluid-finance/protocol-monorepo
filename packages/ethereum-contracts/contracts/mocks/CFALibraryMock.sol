...
/// @dev If a new stream is opened, or an existing one is opened
        ...
        returns (bytes memory newCtx)
    {
     ...
     //if flow exists, update the flow
        ...
        newCtx = cfaV1.updateFlowWithCtx(newCtx, _receiver, _acceptedToken, inFlowRate);
     } 
    else {
      // @dev If there is no existing outflow, then create new flow to equal inflow
        newCtx = cfaV1.createFlowWithCtx(newCtx, _receiver, _acceptedToken, inFlowRate);
      }
    }
...
contract TradeableCashflowMock is ERC721, RedirectAll {
