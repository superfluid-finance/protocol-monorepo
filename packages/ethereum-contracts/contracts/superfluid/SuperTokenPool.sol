...
contract SuperTokenPool is Ownable, ISuperTokenPool {
    ...
    function updateMember(address memberAddr, Unit unit) external
        returns (bool)
    {
        if (Unit.unwrap(unit) < 0) revert SUPER_TOKEN_POOL_NEGATIVE_UNITS_NOT_SUPPORTED();
    ...
    function getClaimable(uint32 time, address memberAddr) public view override
        returns (int256)
    {
        Time t = Time.wrap(time);
    ...
    function claimAll(uint32 time, address memberAddr) public override returns (bool) {
        ...
        // assert(ISuperToken(owner()).shift(address(this), memberAddr, claimable));
    ...
    function operatorSetIndex(PDPoolIndex calldata index) external
    ...
    function operatorConnectMember(uint32 time, address memberAddr, bool doConnect) external
        onlyOwner returns (bool)
    {
        if (doConnect) {
