...
- Merge/port the tooling-branch work into `release_1.16.0-truffle-removal`
...
| `UpgradeFramework.s.sol` bitrots on `2025-11-tooling` | Merge early; rebase onto `release_1.16.0-truffle-removal` |
...
- [x] `yarn build` does not invoke Truffle
- [ ] `yarn test` passes (Foundry **and** Hardhat suites) — CI verification pending
- [x] npm package does **not** contain `build/truffle/`; CHANGELOG documents migration
- [x] `build/bundled-abi.json` still published and correct
- [x] `ops-scripts/` removed; `docs/ops-scripts-migration.md` lists replacements and accepted gaps
- [x] No `npx truffle exec` in repo (workflows, tasks, package scripts)
- [x] `@superfluid-finance/js-sdk` removed from ethereum-contracts; js-sdk package deprecated
- [x] `@decentral.ee/web3-helpers` moved from `dependencies` to `devDependencies` (full removal optional follow-on)
- [x] Root monorepo on Yarn 4 (`nodeLinker: node-modules`) — local verify done; CI green pending

See also [external consumer migration](2026-07-v1.16.0-external-consumers.md).
...
1. Rebase `remotes/origin/2025-11-tooling` onto `release_1.16.0-truffle-removal` and assess `UpgradeFramework.s.sol` merge effort.
