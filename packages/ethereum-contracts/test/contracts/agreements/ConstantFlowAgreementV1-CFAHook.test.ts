...
describe("CFAv1 | CFA Hook Mock Tests", function () {
    ...
    describe("#3 Hardcoded CFA_HOOK_GAS_LIMIT Tests", () => {
        before(async () => {
            await t.beforeTestSuite({
                isTruffle: true,
                nAccounts: 5,
        ...
        it("#3.1 Should return higher gasLimit when CFA_HOOK_GAS_LIMIT is set", async () => {
            const hookGasLimitCFA = await CFAFactory.deploy(
                t.contracts.superfluid.address,
                GoodCFAHookMock.address,
                CFA_HOOK_GAS_LIMIT
            );
            await t.contracts.governance.updateContracts(
                t.contracts.superfluid.address,
                ethers.constants.AddressZero,
                [hookGasLimitCFA.address],
                ethers.constants.AddressZero
            );
            const createFlowCalldata =
                t.agreementHelper.cfaInterface.encodeFunctionData(
                    "createFlow",
                    [superToken.address, bob, "1", "0x"]
                );
            const signer = await ethers.getSigner(alice);
            const populatedTxn = await t.contracts.superfluid
                .connect(signer)
                .populateTransaction.callAgreement(
                    hookGasLimitCFA.address,
                    createFlowCalldata,
                    "0x"
                );
            console.log(populatedTxn.gasLimit?.toString());

            const noHookGasLimitCFA = await CFAFactory.deploy(
                t.contracts.superfluid.address,
                GoodCFAHookMock.address,
                "0"
            );
            await t.contracts.governance.updateContracts(
                t.contracts.superfluid.address,
                ethers.constants.AddressZero,
                [noHookGasLimitCFA.address],
                ethers.constants.AddressZero
            );

            const populatedTxn2 = await t.contracts.superfluid
                .connect(signer)
                .populateTransaction.callAgreement(
                    noHookGasLimitCFA.address,
                    createFlowCalldata,
            ...
            console.log(populatedTxn2.gasLimit?.toString());
            // expect(populatedTxn2.gasLimit).to.be.lt(populatedTxn.gasLimit);
        });
    });
});
