...
const SuperfluidOwnableGovernance = artifacts.require("SuperfluidOwnableGovernance");
...
contract("Superfluid Ownable Governance Contract", (accounts) => {
    ...
    context.only("Upgradable deployment", () => {
        let governance;

        before(async () => {
            const superfluidGovernanceLogic = await SuperfluidOwnableGovernance.new({ from: alice });

            const proxy = await web3tx(
                UUPSProxy.new,
                "Create SuperfluidGovernance proxy"
            )({ from: alice });
            await web3tx(
                proxy.initializeProxy,
                "proxy.initializeProxy"
            )(superfluidGovernanceLogic.address, { from: alice });
            governance = await SuperfluidOwnableGovernance.at(proxy.address);
        });
        // TODO: fix numbering
        // TODO: use context like in Superfluid.test.js?
        describe("#1 upgradability", () => {
            it("#1.1 storage layout", async () => {
                const T = artifacts.require("SuperfluidGovernanceUpgradabilityTester");
            ...
            it("#1.2 proxiable info", async () => {
                assert.equal(
                    ...
                    web3.utils.sha3(
                        "org.superfluid-finance.contracts.SuperfluidGovernance.implementation"
                    )
                );
            });
