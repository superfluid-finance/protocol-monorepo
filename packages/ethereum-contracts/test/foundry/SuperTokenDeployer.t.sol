...
contract SuperTokenDeployerTest is FoundrySuperfluidTester {
    ...
    function testDeployWrapperSuperToken(
        ...
        (TestToken underlyingToken, SuperToken _superToken) =
            superTokenDeployer.deployWrapperSuperToken(_name, _symbol, _decimals, _mintLimit);
        ...
        assertEq(
            _superToken.name(),
            string.concat("Super ", _symbol),
            "SuperTokenDeployer: Super token name not properly set"
        );
        assertEq(
            _superToken.symbol(), string.concat(_symbol, "x"), "SuperTokenDeployer: Super token symbol not properly set"
        );
    ...
    function testDeployNativeAssetSuperToken(string calldata _name, string calldata _symbol) public {
        ISETH nativeAssetSuperToken = superTokenDeployer.deployNativeAssetSuperToken(_name, _symbol);

        // assert native asset super token name/symbol properly set
        assertEq(
            nativeAssetSuperToken.name(), _name, "SuperTokenDeployer: Native asset super token name not properly set"
        );
        assertEq(
            nativeAssetSuperToken.symbol(),
            _symbol,
            "SuperTokenDeployer: Native asset super token symbol not properly set"
        );
    ...
    function testDeployPureSuperToken(string calldata _name, string calldata _symbol, uint256 _initialSupply) public {
        ...
        IPureSuperToken pureSuperToken = superTokenDeployer.deployPureSuperToken(_name, _symbol, _initialSupply);
