...
contract TOGATest is FoundrySuperfluidTester {
     ...
     */
    function _sendPICBid(address sender, ISuperToken token, uint256 bond, int96 exitRate) internal {
        vm.startPrank(sender);
        token.send(address(toga), bond, abi.encode(exitRate));
        vm.stopPrank();
    }

    function _sendPICBid(address sender, ISuperToken token, uint256 bond, bytes memory exitRate) internal {
        vm.startPrank(sender);
        token.send(address(toga), bond, exitRate);
        vm.stopPrank();
    }

    function _shouldDefaultExitRate(uint256 bondAmount) internal pure returns (int96) {
        return int96(int256(bondAmount / (MIN_BOND_DURATION * 4)));
    }

    function _shouldMaxExitRate(uint256 bondAmount) internal pure returns (int96) {
        return int96(int256(bondAmount / MIN_BOND_DURATION)); 
    }

    function _collectRewards(ISuperToken token, int96 flowRate, uint256 time) internal {
    ...
    function testContractSetup() public {
        assertTrue(toga.minBondDuration() == MIN_BOND_DURATION, "minBondDuration");
    }
    ...
    function testAliceBecomesPIC() public {
        ...
        vm.startPrank(alice);
        superToken.send(address(toga), BOND_AMOUNT_1E18, abi.encode(EXIT_RATE_1));

        assertTrue(toga.getCurrentPIC(superToken) == alice);

        (address pic, uint256 bond, int96 exitRate) = toga.getCurrentPICInfo(superToken);
    ...
    function testTOGARegisteredWithERC1820() public {
        ...
        assertTrue(implementer1 == address(toga), "TOGA should be registered as TOGAv1");
        assertTrue(implementer2 == address(toga), "TOGA should be registered as TOGAv2");    
    }
    ...
    function testEnforceMaxExitRateLimit() public {
        ...
        _sendPICBid(alice, superToken, BOND_AMOUNT_1E18, _shouldMaxExitRate(BOND_AMOUNT_1E18) + 1);
    ...
    function testMaxAndDefaultExitRateAreCalculatedCorrectly() public {
        assertTrue(_shouldMaxExitRate(BOND_AMOUNT_1E18) == toga.getMaxExitRateFor(superToken, BOND_AMOUNT_1E18));
        assertTrue(_shouldMaxExitRate(BOND_AMOUNT_2E18) == toga.getMaxExitRateFor(superToken, BOND_AMOUNT_2E18));
        assertTrue(_shouldDefaultExitRate(BOND_AMOUNT_1E18) == toga.getDefaultExitRateFor(superToken, BOND_AMOUNT_1E18));
        assertTrue(_shouldDefaultExitRate(BOND_AMOUNT_2E18) == toga.getDefaultExitRateFor(superToken, BOND_AMOUNT_2E18));
        // the default exit rate needs to be equal or greater than the max exit rate
        assertTrue(toga.getMaxExitRateFor(superToken, BOND_AMOUNT_2E18) >= toga.getDefaultExitRateFor(superToken, BOND_AMOUNT_2E18));
    }
    ...
    function testUseDefaultExitRateAsFallbackIfNoExitRateSpecified() public {
        ...
        _assetNetflow(superToken, alice, toga.getDefaultExitRateFor(superToken, BOND_AMOUNT_1E18));
    }
    ...
    function testFirstBidderGetsTokensPreOwnedByContract() public {
        ...
        assertTrue(superToken.balanceOf(alice) == (alicePreOutbidBal + aliceBond));
    }
    ...
    function testCurrentPICCanIncreaseBond() public {
        ...
        assertTrue(toga.getCurrentPIC(superToken) == alice);
        _assetNetflow(superToken, alice, 0);
        assertTrue(superToken.balanceOf(alice) == (aliceIntermediateBal - BOND_AMOUNT_1E18));
        (,uint256 bond,) = toga.getCurrentPICInfo(superToken);
