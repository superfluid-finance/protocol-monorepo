...
contract SemanticMoneyTest is Test {
    ...
    function testSettleTwice(BasicParticle memory p, uint16 m) external {
    ...
    function testShift2Shift2(uint16 m1, int64 x1, uint16 m2, int64 x2, uint16 m3) external {
    ...
    function testFlow2Shift2(uint16 m1, int64 r1, uint16 m2, int64 x2, uint16 m3) external {
        BasicParticle memory a;
        BasicParticle memory b;
        Time t1 = Time.wrap(m1);
        Time t2 = t1.add(Time.wrap(m2));
    ...
    function testShift2Flow2(uint16 m1, int64 x1, uint16 m2, int64 r2, uint16 m3) external {
        BasicParticle memory a;
        BasicParticle memory b;
        Time t1 = Time.wrap(m1);
        Time t2 = t1.add(Time.wrap(m2));
