...
class ( Default ac
    ...
    --
    --   1. An updated ~ac'~.  2. A functorful delta of agreement monetary unit data ~mudsΔ~, which then can be
    --   monoid-appended to existing ~mudΔ~.  This is what can make an agreement scalable.
    ...
    functorizeAgreementOperationOutput
        ...
        -> f (AnySemigroupMonetaryUnitDataClass sft)
