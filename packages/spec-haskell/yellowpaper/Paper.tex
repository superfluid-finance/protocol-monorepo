...
\centerline{\textbf{Axioms of restricted money distributions}}
...
This allows us to impose the law of conservation of value on payment primitives, then prove inductively that such
restricted money distribution obey the law of conservation of value too.

\centerline{\textbf{Proof of restricted money distributions satisfying the law of conservation of value}}
...
A \textit{index} has a function $\rho$ which produces a real number for each monetary unit:

\begin{equation}
    \rho :: k \rightarrow u \rightarrow \mathbb{R}
\end{equation}

And it must satisfy the following law:

\begin{equation}
    \displaystyle \sum_{u \in U} \rho\ u = 1
\end{equation}

Semantically, it represents a proportion of each money unit, and they must add up to 1.

Now let's generalize the one to one payment primitives using index abstraction. We use symbol $k$ for the indexes, and
$I$ subscript for the indexed versions of the primitives.
...
Now one to one payment primitives can be redefined using \textit{universal index} ($ku_{any}$) define as such:

\begin{equation}
    \rho\ ku_{a} = \lambda u \rightarrow if\ u\ =\ u_a\ then\ 1\ else\ 0
\end{equation}

It means that it's an index universally available for any monetary unit, and its proportion is always 1 for that
monetary unit.
