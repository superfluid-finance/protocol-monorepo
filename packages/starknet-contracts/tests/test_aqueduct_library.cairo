...
func test_cal_new_flowrate_A{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (newFlowRateA) = AqueductLibrary.cal_new_flowrate_A(totalA, totalB, oldFlowRateB, newFlowRateB);
    let (expected_newFlowRateA, _) = unsigned_div_rem(totalA * newFlowRateB, (totalB + newFlowRateB) - oldFlowRateB);
...
func test_cal_new_flowrate_B{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (newFlowRateB) = AqueductLibrary.cal_new_flowrate_B(totalA, totalB, oldFlowRateA, newFlowRateA);
    let (expected_newFlowRateB, _) = unsigned_div_rem(totalB * newFlowRateA, (totalA + newFlowRateA) - oldFlowRateA);
