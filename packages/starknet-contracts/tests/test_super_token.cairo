...
//     %{ stop_warp = warp(ids.t2, ids.supertoken_contract_address) %}
//     let (distributionFlowRate) = ISuperTokenPool.getDistributionFlowRate(contract_address=pool);
//     let (pendingDistributionFlowRate) = ISuperTokenPool.getPendingDistributionFlowRate(contract_address=pool);
//     let (netFlowRateOfAccount1) = ISuperToken.getNetFlowRate(
//         contract_address=supertoken_contract_address, account=account1
//     );
//     let (netFlowRateOfAccount2) = ISuperToken.getNetFlowRate(
...
//     assert netFlowRateOfAccount1 = -actualFlowRate;
//     assert netFlowRateOfAccount2 = flowRatePerUnit * u1;
//     assert netFlowRateOfAccount3 = pendingDistributionFlowRate;
...
//     assert balanceOfAccount1 = 0;
//     assert balanceOfAccount2 = (t2 - timestamp) * netFlowRateOfAccount2;
//     assert balanceOfAccount3 = (t2 - timestamp) * pendingDistributionFlowRate;
//     %{ stop_warp() %}
//     return ();
// }

@external
func setup_distribute2to1_flow{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr
}() {
    %{
        given(
...
func test_distribute2to1_flow{
    ...
    %{ stop_warp = warp(ids.t2, ids.supertoken_contract_address) %}
    let (distributionFlowRateForPool1) = ISuperTokenPool.getDistributionFlowRate(contract_address=pool1);
    let (distributionFlowRateForPool2) = ISuperTokenPool.getDistributionFlowRate(contract_address=pool2);

    let (pendingDistributionFlowRateForPool1) = ISuperTokenPool.getPendingDistributionFlowRate(contract_address=pool1);
    let (pendingDistributionFlowRateForPool2) = ISuperTokenPool.getPendingDistributionFlowRate(contract_address=pool2);
    ...
    assert netFlowRateOfAccount3 =  ((flowRatePerUnitForPool1 * u1) + (flowRatePerUnitForPool2 * u2));
