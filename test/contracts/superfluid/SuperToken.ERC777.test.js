...
contract("SuperToken's ERC777 implementation", accounts => {
    ...
    beforeEach(async function () {
        ...
        await web3tx(this.token.upgrade, "Upgrade initialSupply amount of token for holder")(
            initialSupply, {
                from: holder
            }
        );
    ...
    describe("operator management", function () {
        ...
        it("reverts when self-authorizing", async function () {
            await expectRevert(
                this.token.authorizeOperator(holder, { from: holder }), "ERC777Operators: authorizing self as operator",
            );
        });

        it("reverts when self-revoking", async function () {
            await expectRevert(
                this.token.revokeOperator(holder, { from: holder }), "ERC777Operators: revoking self as operator",
            );
        });
            ...
            describe("with no ERC777TokensRecipient implementer", function () {
                describe("with contract recipient", function () {
                    ...
                    it("mint (internal) reverts", async function () {
                        await expectRevert(
                            this.token.mintInternal(
                                this.recipient, amount, data, operatorData,
                                {from:operator}
                            ),
                            "SuperToken: not an ERC777TokensRecipient",
                        );
                    ...
                    it("(ERC20) transfer succeeds", async function () {
                        await web3tx(this.token.upgrade, "SuperToken.upgrade 2 from holder") (
                            toWad(2), {
                                from: holder
                            });
