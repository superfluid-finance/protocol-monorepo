...
contract("Super ETH (SETH) Contract", accounts => {
    ...
    beforeEach(async () => {
        ...
        seth = await SETH.at(
            (await SETHProxy.new(weth.address)).address
        );
        await web3tx(superTokenFactory.initializeCustomSuperToken, "initializeCustomSuperToken")(
            seth.address
        );
    ...
    it("#1.1 upgrade by WETH", async () => {
        await web3tx(weth.deposit, "weth.deposit 1wad")({
            from: alice,
            value: toWad(1)
        });
        assert.equal(
        ...
        await web3tx(seth.upgrade, "seth.upgrade by alice")(
            toWad(1), {
                from: alice
            }
        );
        ...
        await web3tx(seth.downgrade, "seth.downgrade by alice")(
            toWad(1), {
                from: alice
            }
        );
    ...
    it("#1.2 upgrade by ETH", async () => {
        await web3tx(seth.upgradeByETH, "seth.upgradeByETH by alice")({
            from: alice,
            value: toWad(1)
        });
        assert.equal(
